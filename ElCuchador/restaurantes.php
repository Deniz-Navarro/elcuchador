            ...
            <div class="col-lg-8 mt-2">
                <nav id="navbar-example2" class="navbar px-3 justify-content-center">
                    <ul class="nav nav-pills">
                        <li class="nav-item">
                            <a class="nav-link text-dark" href="#fam">Familiar</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link text-dark" href="#buf">Buffet</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link text-dark" href="#cr">Comida Rapida</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link text-dark" href="#tem">Tematico</a>
                        </li>
                    </ul>
                </nav>
                <br><br>
                <div data-bs-spy="scroll" data-bs-target="#navbar-example2" data-bs-offset="0" tabindex="0">
                    <h4 id="fam">Familiar</h4>
                        <div class="row">
                            <?php
                            $query = mysqli_query($conn,"SELECT * FROM publicaciones WHERE tcomida = 'Familiar'");
                            card($query)
                            ?>
                        </div>
                    <h4 id="buf">Buffet</h4>
                        <div class="row">
                            <?php
                            $query = mysqli_query($conn,"SELECT * FROM publicaciones WHERE tcomida = 'Buffet'");
                            card($query)
                            ?>
                        </div>
                    <h4 id="cr">Comida Rapida</h4>
                        <div class="row">
                            <?php
                            $query = mysqli_query($conn,"SELECT * FROM publicaciones WHERE tcomida = 'Comida Rapida'");
                            card($query)
                            ?>
                        </div>
                    <h4 id="tem">Tematico</h4>
                        <div class="row">
                            <?php
                            $query = mysqli_query($conn,"SELECT * FROM publicaciones WHERE tcomida = 'Tematico'");
                            card($query)
                            ?>
                        </div>
                </div>
